...
public class ClimacellClient {
	...
	public void postConstruct() {
		SimpleModule module =
				  new SimpleModule("TemperatureDeserializer", new Version(1, 0, 0, null, null, null));
	...
	public Temperature getCurrentTemperature() {
		...
		UriComponentsBuilder builder = UriComponentsBuilder.fromHttpUrl(urlRealTime)
				.queryParam("lat", lat)
				.queryParam("lon", lon)
				.queryParam("fields", "temp");
	...
	public List<Temperature> getHistoricalTemperatureForDate(LocalDate date) {
		...
		headers.set("apikey", key);
		headers.set("Accept", MediaType.APPLICATION_JSON_VALUE);
		UriComponentsBuilder builder = UriComponentsBuilder.fromHttpUrl(urlHistorical)
				.queryParam("lat", lat)
				.queryParam("lon", lon)
				.queryParam("start_time", date.toString())
				.queryParam("end_time", date.plusDays(1).toString())
				.queryParam("fields", "temp");
		HttpEntity<?> entity = new HttpEntity<>(headers);
		HttpEntity<String> response = restTemplate.exchange(
		        builder.toUriString(), 
		...
		try {
			...
			return temperatures.stream().filter(t -> t.getObservationTime().getMinute() == 20).collect(Collectors.toList());
		} catch (JsonProcessingException e) {
	...
	public List<Temperature> convertJsonToTemperatureList(String json) throws JsonMappingException, JsonProcessingException {
